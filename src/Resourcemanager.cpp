...
template<>
void Resource::Load<sf::Texture>(const std::string& path)
{
	if(myType == NONE)
	{
		myType = TEXTURE;
...
template<>
sf::Texture& Resource::Get<sf::Texture>() const
{
	if(myType != TEXTURE) throw std::logic_error("You tried to load an image from a resource which is no image.");
